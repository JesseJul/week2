...
const updateCat = async (cat) => {
  const [row] = await promisePool.execute('UPDATE `wop_cat` SET `name`=?, `age`=?, `weight`=7 WHERE cat_id=?', [cat.name, cat.age, cat, cat.weight, cat.id]);
  console.log('update row', row);
  return true
}
const deleteCat = async (id) => {
  const [row] = await promisePool.execute('DELETE `wop_cat` WHERE cat_id=?', [id]);
